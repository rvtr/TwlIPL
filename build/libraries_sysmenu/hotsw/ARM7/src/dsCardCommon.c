...
HotSwState ReadPageGame2(CardBootData *cbd, u32 start_addr, void* buf, u32 size,u32 param)
{
    ...
    for(i=0; i<loop; i++){
        ...
        // コマンド作成
		cndLE.dw  = HSWOP_G_OP_RD_PAGE;
		cndLE.dw |= (page + i) << 32;//R4
		cndLE.dw |= ( ((start_addr & 0x0ff) << 24) | 0x00120000);//R4
...
void SendMgcComnd(u32 cmd_h,u32 cmd_l,u32 param){
    ...
    // DMA転送の準備
#ifdef USE_NEW_DMA
    HOTSW_NDmaCopy_Card( HOTSW_NDMA_NO, (u32 *)HOTSW_MCD1, &res, 4 );
#else
    HOTSW_DmaCopy32_Card( HOTSW_DMA_NO, (u32 *)HOTSW_MCD1, &res, 4 );
#endif

    cndLE.dw = (u64)cmd_h << 32;
    cndLE.dw |= cmd_l;
...
//マジコン調査
//DSTWO特殊コマンドでpageリード
HotSwState ReadMgc(u32 param, u32 start_addr, void* buf, u32 size)
{
    u32 		loop, counter=0;
	u64			i, page;
	GCDCmd64	cndLE;
