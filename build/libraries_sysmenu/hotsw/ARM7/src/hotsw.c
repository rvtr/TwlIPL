...
static BOOL ChangeGameMode(void)
{
    ...
    state = s_funcTable[s_isRomEmu].SetPNG_S(&s_cbData);
    SetMCSCR();
    state  = s_funcTable[s_isRomEmu].ReadID_S(&s_cbData); // 追加
    state  = s_funcTable[s_isRomEmu].ReadSegment_S(&s_cbData); // 追加
    state = s_funcTable[s_isRomEmu].ChangeMode_S(&s_cbData);

    // ---------------------- Game Mode ----------------------
    state = ReadIDGame(&s_cbData);

    state = LoadBannerData(); // 追加
    state = CheckCardAuthCode(); // 追加
