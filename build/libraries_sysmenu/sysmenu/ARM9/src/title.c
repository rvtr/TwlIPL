            ...
            if (step >= 4){//取得できたらリスト格納
                if (mode ==0){//pre
                    if (lst < MGCN_CMD_MAX-1){
                        ...
                        pre_list[lst][1] = param2 & 0x3fffffff;//フラグに使うので上位０固定
                    ...
                    if (lst < MGCN_CMD_MAX-1){
                        ...
                        pst_list[lst][1] = param2 & 0x3fffffff;//フラグに使うので上位０固定
...
static void SYSMi_LoadTitleThreadFunc( TitleProperty *pBootTitle )
{
	...
	case LAUNCHER_BOOTTYPE_ROM:
        ...
        //コマンドファイルがあれば読み込む
            ...
            pre_cmd[0] = 0xff;
            pst_cmd[0] = 0xff;
            pwork = SYSM_Alloc( 1024*32 );//とりあえず32K
            if (pwork){
...
//製品(ALXX)と同じ読み方でないと正体出さないマジコン対策
            {
              int n = 0;
              u32 sz,flg;
              u8* pwork = NULL;
              pwork = SYSM_Alloc( 0x20000 );//とりあえず、DSTWOは128Kあれば足りる
              if (pwork){
...
//#endif
...

           {
              ...
              FSFile src;
              pwork = SYSM_Alloc( 0x20000 );//とりあえず、DSTWOは128Kあれば足りる
              if (pwork && (filename[0] != 0)){
                  //リストがあれば 
                  //ROM吸出し後(オーバレイチェック前)にオーバレイなど個別に読む
                  while(n < MGCN_CMD_MAX){
                      ...
                      if (pst_cmd[n] == 'C'){//コマンド
                          ...
                          if (pre_cmd[n] == 'M')flg = 1;//read & out to SD
                          if (pre_cmd[n] == 'D')flg = 3;//special read & out to SD
                          else flg = 0;//read
                      ...
                      //if(flg){    //出力
                      if(flg & 1){    //出力
                          STD_TSNPrintf( filename, 64, "sdmc:/preload_%d_%x_%x.dat",n,pst_list[n][0],sz);
                          FS_InitFile( &dest );
                          (void)FS_CreateFile(filename, FS_PERMIT_W | FS_PERMIT_R);
                          FS_OpenFileEx( &dest, filename, FS_FILEMODE_W );
                          FS_WriteFile( &dest, pwork, (int)sz );
