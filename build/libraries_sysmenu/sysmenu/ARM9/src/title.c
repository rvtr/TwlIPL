...
void fMGCN_GetList(u8* buff,int size)
{
    ...
    while(n < size){
        ...
        }else if (flg == 2){//オフセット取得
            if ((buff[n] == ' ') || (buff[n] == ',')){//セパレータ
                if( i >0 ){i=0; flg = 3;}//next
            ...
            if (flg >= 4){//取得できたらリスト格納
                ...
                if (mode ==0){
                    if (lst < MGCN_CMD_MAX-1){
                        pre_list[lst][0] = ofst;
                        pre_list[lst][1] = len;
                        pre_list[lst+1][0] = 0xffffffff;
                    }
                }else{
                    if (lst < MGCN_CMD_MAX*2-1){
                        pst_list[lst][0] = ofst;
                        pst_list[lst][1] = len;
                        pst_list[lst+1][0] = 0xffffffff;
                    }
                }
                if (flg == 4)flg = 1;//改行まで進める
                else flg = 0;//先頭
                lst++;
            }
        }else{
            if (buff[n] == '#')flg = 1;//コメント
            else if (buff[n] == '<'){
                if ((n+1)<size){
...
//製品(ALXX)と同じ読み方でないと正体出さないマジコン対策
            {
              ...
              if (pwork){
                  while(n < MGCN_CMD_MAX){
                      if(pre_list[n][0] == 0xffffffff )break;//end
                      sz = pre_list[n][1] & 0xfffffffe;
                      if( sz > 0x20000 ){
                      ...
                      HOTSW_ReadCardData( (void*)pre_list[n][0], pwork,sz);
                
                      if(pre_list[n][1] & 1 ){    //サイズ指定 の b0 が立ってたら出力
...
//#endif
            ...
            if( mode_save_to_sd != 0 )
            {
                ...
                if(FS_OpenFileEx( &dest, filename, FS_FILEMODE_W ))
                {
                   ...
                   //エラーのとき filename=""
                    ...
                    FS_InitFile( &dest );
                    (void)FS_CreateFile("sdmc:/open_fail_rom", FS_PERMIT_W | FS_PERMIT_R);
                    if(FS_OpenFileEx( &dest,"sdmc:/open_fail_rom", FS_FILEMODE_W ))
                    FS_CloseFile( &dest );
                    filename[0] = 0;
                }
            }

           {
              ...
              if (pwork && (filename[0] != 0)){
                  ...
                  while(n < MGCN_CMD_MAX){
                    if(pst_list[n][0] == 0xffffffff )break;//end
                    sz = pst_list[n][1] & 0xfffffffe;
                    if( sz > 0x20000 ){
                    ...
                    HOTSW_ReadCardData( (void*)pst_list[n][0], pwork,sz);
                
                    if(pst_list[n][1] & 1 ){    //サイズ指定 の b0 が立ってたら出力
                        STD_TSNPrintf( filename, 64, "sdmc:/postload_%d_%x_%x.dat",n,pst_list[n][0],sz);
                        FS_InitFile( &src );
                        (void)FS_CreateFile(filename, FS_PERMIT_W | FS_PERMIT_R);
                        FS_OpenFileEx( &src, filename, FS_FILEMODE_W );
                        FS_WriteFile( &src, pwork, sz );
                        FS_CloseFile( &src );
                    }
                    n++;
                  }
                  ...
                  {
                       n = 0;
                       while(n < MGCN_CMD_MAX){//プリ
                           if(pre_list[n][0] == 0xffffffff )break;//end
                           if(pre_list[n][1] & 1 ){//出力あり
                               sz = pre_list[n][1] & 0xfffffffe;
                       ...
                       n = 0;
                       while(n < MGCN_CMD_MAX){//ポスト
                           if(pst_list[n][0] == 0xffffffff )break;//end
                           if(pst_list[n][1] & 1 ){//出力あり
                               sz = pst_list[n][1] & 0xfffffffe;
                               if((sz > 0x20000 ) || (sz < 0))sz = 0x20000;
                               STD_TSNPrintf( filename, 64, "sdmc:/postload_%d_%x_%x.dat",n,pst_list[n][0],sz);
                               FS_InitFile( &src );
                       ...
                       FS_InitFile( &src );
                       (void)FS_CreateFile("sdmc:/open_fail_rom2", FS_PERMIT_W | FS_PERMIT_R);
                       if(FS_OpenFileEx( &src,"sdmc:/open_fail_rom2", FS_FILEMODE_W ))
