...
#ifdef MGCN_TEST_ENABLE
void fMGCN_GetList(u8 *work,int size);
u8 fMGCN_Char2Hex(u8 ch);
#endif
...
#ifdef MGCN_TEST_ENABLE
//  --------- マジコン調査
#define MGCN_CMD_MAX 64
u32 pre_list[MGCN_CMD_MAX][2];// プリロードリスト: offset,size
u32 pst_list[MGCN_CMD_MAX][2];// ポストロードリスト: offset,size
u8 pre_cmd[MGCN_CMD_MAX];
u8 pst_cmd[MGCN_CMD_MAX];
s32 mgc_rom_size;

u32 mgcn_cfg;//コンフィグ .. 偽装調査、対策検討で使用
//+0
//  b7:Hash Phase2 OVL Out enable
//  b0:Read Overlay size
//  b1:Write Overlay Size
//+1:Max Overlay of full size;
//+2
//常駐部分ロード直後にROM吸出
#define mgcn_cfg_flg_rom0 1
#endif
...
//プレ・ポストリード用 リスト生成
// パラメータ2の下位2bitをフラグに使用
// サイズやコマンドパラメータで使う場合はマズイ(2013.3 現在で該当なし)
// buff: SDから読んだ設定ファイル(ctrl.cfg)格納アドレス
void fMGCN_GetList(u8* buff,int size)
{
    int n =0;
    int lst = 0,i=0,mode=0;
    u8 cmd,d,step=0;
    u32 param1,param2;
    ...
    while(n < size){
        if (step == 1){//改行まで移動
            if (buff[n] == 0x0a){
                ...
                if ( ((n+1)<size) && (buff[n+1] == 0x0a) )n++;
                step = 0;
            }
        }else if (step == 2){//パラメータ1：オフセット or コマンド上位
            if ((buff[n] == ' ') || (buff[n] == ',')){//セパレータ
                if( i >0 ){i=0; step = 3;}//next
            }else if (buff[n] == 0x0a){//改行
                step = 0;
            }else if (buff[n] == 0x0d){//改行(CR+LF)
                if ( ((n+1)<size) && (buff[n+1] == 0x0a))n++;
                step = 0;
            }else{
                if (i >7) step = 1;//無効行
                else{
                    d = fMGCN_Char2Hex(buff[n]);
                    if ((d >=0) && (d <= 0x0f)){
                        param1 = (param1 << 4) | d;
                        i++;
                    }else step = 1;//無効行
        ...
        }else if (step == 3){//パラメータ２:サイズ or コマンド下位
            if ((buff[n] == ' ') || (buff[n] == ',')){//セパレータ
                if( i >0 ) step = 4;
            }else if (buff[n] == 0x0a){//改行
                if( i >0 ) step = 5;
            }else if (buff[n] == 0x0d){//改行
                if ( ((n+1)<size) && (buff[n+1] == 0x0a))n++;
                if( i >0 ) step = 5;
            }else{
                if (i >7) step = 1;//無効行
            ...
            if (step >= 4){//取得できたらリスト格納
                if (cmd == 'S'){//size
                    mgc_rom_size = param2 & 0x00fff;
                }else{
                  if (mode ==0){//pre
                    if (lst < MGCN_CMD_MAX-1){
                        pre_cmd[lst] = cmd;
                        pre_cmd[lst+1] = 0xff;//end mark
                        pre_list[lst][0] = param1;
                        pre_list[lst][1] = param2 & 0x3fffffff;//上位bitはフラグに使う
                        pre_list[lst+1][0] = 0xffffffff;//end mark
                    }
                  }else{//post
                    if (lst < MGCN_CMD_MAX-1){
                        pst_cmd[lst] = cmd;
                        pst_cmd[lst+1] = 0xff;//end mark
                        pst_list[lst][0] = param1;
                        pst_list[lst][1] = param2 & 0x3fffffff;//上位bitはフラグに使う
                        pst_list[lst+1][0] = 0xffffffff;//end mark
                    }
                  }
                }
                if (step == 4)step = 1;//改行まで進める
                else step = 0;//先頭
                lst++;
            }
        }else{
            if (buff[n] == '#')step = 1;//コメント
            else if (buff[n] == '<'){//セクション
                if ((n+1)<size){
                    if(buff[n+1] == 'B'){pre_list[0][0]=0xffffffff;lst = 0;mode = 0;}//<BEFOR>
                    else if(buff[n+1] == 'A'){pst_list[0][0]=0xffffffff;lst = 0;mode=1;}//<AFTER>
                    else if(buff[n+1] == 'E')break;//<END>
                    else step = 1;//無効行
                }
            }else if ((buff[n] == 'R') || (buff[n] == 'M')
                           || (buff[n] == 'C') || (buff[n] == 'D')){//パラメータ 2つ
                step = 2;
                param1 = 0;
                param2 = 0;
                i =0;
                cmd = buff[n];
            }else if (buff[n] == 'S'){//サイズ
                step = 3;
                param1 = 0;
                param2 = 0;
                i =0;
...
static void SYSMi_LoadTitleThreadFunc( TitleProperty *pBootTitle )
{
	...
	case LAUNCHER_BOOTTYPE_ROM:
        ...
        //設定ファイルがあれば読み込む
            mgcn_cfg = mgcn_cfg_flg_rom0 << 16;//デフォ
		    STD_TSNPrintf( filename, 64, "sdmc:/ctrl.cfg");
	        FS_InitFile( &dest );
	        FS_OpenFileEx( &dest, filename, FS_FILEMODE_R );
	        FS_ReadFile( &dest,&mgcn_cfg,sizeof(mgcn_cfg));
	        FS_CloseFile( &dest );

        //コマンドファイルがあれば読み込む
            mgc_rom_size = -1;
            pre_list[0][0] = 0xffffffff;//NULL
            pst_list[0][0] = 0xffffffff;//NULL
            pre_cmd[0] = 0xff;
            pst_cmd[0] = 0xff;
            pwork = SYSM_Alloc( 1024*64 );//とりあえず64K
            if (pwork){
		        STD_TSNPrintf( filename, 64, "sdmc:/accs_lst.txt");
	            FS_InitFile( &dest );
	            if (FS_OpenFileEx( &dest, filename, FS_FILEMODE_R )){
	                size = FS_ReadFile( &dest,pwork,1024*32);
	                FS_CloseFile( &dest );
                    if (size > 4)fMGCN_GetList(pwork,size);
	            }else{
	            
	            }
                SYSM_Free(pwork);
            }else{
                STD_TSNPrintf( filename, 64, "sdmc:/alloc_error");
                FS_InitFile( &dest );
...
/*
                STD_TSNPrintf( filename, 64, "sdmc:/list");
                FS_InitFile( &dest );
                (void)FS_CreateFile(filename, FS_PERMIT_W | FS_PERMIT_R);
                if(FS_OpenFileEx( &dest, filename, FS_FILEMODE_W ))
                FS_WriteFile( &dest, pre_list,256 );
                FS_WriteFile( &dest, pst_list,256 );
                FS_CloseFile( &dest );
*/
        }
...
// SDにひととおり吐く
#ifdef MGCN_TEST_ENABLE
        if(( mode_save_to_sd != 0) && isCardApp)//Nandアプリ起動は無視するようカード限定
#endif
        {
            int loop;
            ...
            u8* pcfg = (u8*)&mgcn_cfg;
            int readsize;
          if( pcfg[2] & mgcn_cfg_flg_rom0 )//ROM出力フラグ
#endif
          {

#ifdef MGCN_TEST_ENABLE
//リストの順でロード:ROM吸い出し前
//ROM吸い出す前にオーバレイなど個別に吸い出す
//製品と同じ読み方でないと正体出さないマジコン対策
#define MGCN_BUFF_SIZE 0x40000
            {
              int n = 0;
              u32 sz,flg;
              u8* pwork = NULL;
              pwork = SYSM_Alloc( MGCN_BUFF_SIZE );
              if (pwork){
                  while(n < MGCN_CMD_MAX){
                      if((pre_cmd[n] == 0xff) || (pre_list[n][0] == 0xffffffff ))break;//end
                      sz = pre_list[n][1];
                      if (pre_cmd[n] == 'C'){//コマンド
                          flg = 2;
                      }else{//リード
                          if( sz > MGCN_BUFF_SIZE ){
                              STD_TSNPrintf( filename, 64, "sdmc:/preload_%d_buff_over",n);
                              FS_InitFile( &dest );
                              (void)FS_CreateFile(filename, FS_PERMIT_W | FS_PERMIT_R);
                              if(FS_OpenFileEx( &dest, filename, FS_FILEMODE_W ))
                              FS_CloseFile( &dest );
                          ...
                          if (pre_cmd[n] == 'M')flg = 1;//read & out to SD
                          else if (pre_cmd[n] == 'D')flg = 3;//special read & out to SD
                          else flg = 0;//read
                          
                      }
                      //２番目パラメータのb31,b30をフラグにあてる
                      HOTSW_ReadCardData( (void*)pre_list[n][0], pwork,sz | (flg << 30));
                      
                      //if(flg){    //出力
                      if(flg & 1){    //出力
                          STD_TSNPrintf( filename, 64, "sdmc:/preload_%d_%x_%x.dat",n,pre_list[n][0],sz);
                          FS_InitFile( &dest );
                          (void)FS_CreateFile(filename, FS_PERMIT_W | FS_PERMIT_R);
                          FS_OpenFileEx( &dest, filename, FS_FILEMODE_W );
                          FS_WriteFile( &dest, pwork, (int)sz );
                          FS_CloseFile( &dest );
                      }
                      n++;
                  }
...
//#ifdef MGCN_TEST_ENABLE
//            STD_TSNPrintf( filename, 64, "sdmc:/%c%c%c%c_ARM9FLX_%x_%x.dat", head->s.game_code[0],head->s.game_code[1],head->s.game_code[2],head->s.game_code[3],
//                source[region_arm9_ntr],length[region_arm9_ntr]);
//#else
...
//#ifdef MGCN_TEST_ENABLE
//            STD_TSNPrintf( filename, 64, "sdmc:/%c%c%c%c_ARM7FLX_%x_%x.dat", head->s.game_code[0],head->s.game_code[1],head->s.game_code[2],head->s.game_code[3],
//               source[region_arm7_ntr],length[region_arm7_ntr]);
//#else
...
//#endif
            ...
            {
#ifndef MGCN_TEST_ENABLE
                int readsize = (mode_save_to_sd==1 ? 32 : 256);
#else
                if (mgc_rom_size >= 0)readsize = mgc_rom_size;
                else readsize = (mode_save_to_sd==1 ? 32 : 256);
              if (readsize > 0){
#endif
...
#else 
                   //エラーのとき filename=""
                    if((card_read_state != CARD_READ_SUCCESS)){
                        FS_InitFile( &dest );
                        (void)FS_CreateFile("sdmc:/read_fail_rom", FS_PERMIT_W | FS_PERMIT_R);
                        if(FS_OpenFileEx( &dest, "sdmc:/read_fail_rom", FS_FILEMODE_W ))
                        FS_CloseFile( &dest );
                        filename[0] = 0;
                    }
                }else{
                    FS_InitFile( &dest );
                    (void)FS_CreateFile("sdmc:/create_fail_rom", FS_PERMIT_W | FS_PERMIT_R);
                    if(FS_OpenFileEx( &dest,"sdmc:/create_fail_rom", FS_FILEMODE_W ))
                    FS_CloseFile( &dest );
                    filename[0] = 0;
                }
              }
...

           {
              ...
              if ( pwork){
                  while(n < MGCN_CMD_MAX){
                      if((pst_cmd[n] == 0xff) || (pst_list[n][0] == 0xffffffff ))break;//end
                      sz = pst_list[n][1];
                      if (pst_cmd[n] == 'C'){//コマンド
                          flg = 2;
                      }else{//リード
                          if( sz > MGCN_BUFF_SIZE ){
                              STD_TSNPrintf( filename, 64, "sdmc:/postload_%d_buff_over",n);
                              FS_InitFile( &dest );
                              (void)FS_CreateFile(filename, FS_PERMIT_W | FS_PERMIT_R);
                              if(FS_OpenFileEx( &dest, filename, FS_FILEMODE_W ))
                              FS_CloseFile( &dest );
                          ...
                          if (pst_cmd[n] == 'M')flg = 1;//read & out to SD
                          else if (pst_cmd[n] == 'D')flg = 3;//special read & out to SD
                          else flg = 0;//read
                          
                      }
                      //２番目パラメータのb31,b30をフラグにあてる
                      HOTSW_ReadCardData( (void*)pst_list[n][0], pwork,sz | (flg << 30));
                      
                      //if(flg){    //出力
                      if(flg & 1){    //出力
                          STD_TSNPrintf( filename, 64, "sdmc:/postload_%d_%x_%x.dat",n,pst_list[n][0],sz);
                          FS_InitFile( &dest );
                          (void)FS_CreateFile(filename, FS_PERMIT_W | FS_PERMIT_R);
                          FS_OpenFileEx( &dest, filename, FS_FILEMODE_W );
                          FS_WriteFile( &dest, pwork, (int)sz );
                  ...
                  //ROMイメージに合成
                  //readsize = (mode_save_to_sd==1 ? 32 : 256);
                 if ((readsize > 0) && (pre_cmd[0] != 0xff)){
                  STD_TSNPrintf(filename,64,"sdmc:/%c%c%c%c_CARD%dMbit.dat",head->s.game_code[0],head->s.game_code[1],head->s.game_code[2],head->s.game_code[3],readsize);
                  FS_InitFile( &dest );
                  if(FS_OpenFileEx( &dest, filename, FS_FILEMODE_W | FS_FILEMODE_R))
                  {
                       n = 0;
                       while(n < MGCN_CMD_MAX){//プリロード
                           if((pre_cmd[n] == 0xff)||(pre_list[n][0] == 0xffffffff ))break;//end
                           if(pre_cmd[n] == 'M'){
                               sz = pre_list[n][1];
                               if((sz >  MGCN_BUFF_SIZE) || (sz < 0))sz = MGCN_BUFF_SIZE;
                               STD_TSNPrintf( filename, 64, "sdmc:/preload_%d_%x_%x.dat",n,pre_list[n][0],sz);
                               FS_InitFile( &src );
                               if (FS_OpenFileEx( &src, filename, FS_FILEMODE_R )){
                                   FS_ReadFile( &src, pwork, sz );
                                   FS_CloseFile( &src );
        	                       if (FS_SeekFile(&dest, pre_list[n][0], FS_SEEK_SET))
                                   {
                                       FS_WriteFile( &dest, pwork, sz );
                                   }else{
                                       STD_TSNPrintf( filename, 64, "sdmc:/seek_error_pre%d",n);
                                       FS_InitFile( &src );
                                       (void)FS_CreateFile(filename, FS_PERMIT_W | FS_PERMIT_R);
                                       if(FS_OpenFileEx( &src, filename, FS_FILEMODE_W ))
                                       FS_CloseFile( &src );
                                   }
                               }else{
                                   STD_TSNPrintf( filename, 64, "sdmc:/open_fail_pre%d",n);
                                   FS_InitFile( &src );
                                   (void)FS_CreateFile(filename, FS_PERMIT_W | FS_PERMIT_R);
                                   if(FS_OpenFileEx( &src, filename, FS_FILEMODE_W ))
                                   FS_CloseFile( &src );
                       ...
                       while(n < MGCN_CMD_MAX){//ポストロード
                           ...
                           if(pst_cmd[n] == 'M'){
                               ...
                               if (FS_OpenFileEx( &src, filename, FS_FILEMODE_R )){
                                   FS_ReadFile( &src, pwork, sz );
                                   FS_CloseFile( &src );
        	                       if (FS_SeekFile(&dest, pst_list[n][0], FS_SEEK_SET))
                                   {
                                       FS_WriteFile( &dest, pwork, sz );
                                   }else{
                                       STD_TSNPrintf( filename, 64, "sdmc:/seek_error_post%d",n);
                                       FS_InitFile( &src );
                                       (void)FS_CreateFile(filename, FS_PERMIT_W | FS_PERMIT_R);
                                       if(FS_OpenFileEx( &src, filename, FS_FILEMODE_W ))
                                       FS_CloseFile( &src );
                                   ...
                                   STD_TSNPrintf( filename, 64, "sdmc:/open_fail_post%d",n);
                                   FS_InitFile( &src );
                                   (void)FS_CreateFile(filename, FS_PERMIT_W | FS_PERMIT_R);
                                   if(FS_OpenFileEx( &src, filename, FS_FILEMODE_W ))
                                   FS_CloseFile( &src );
                       ...
                       FS_InitFile( &src );
                       (void)FS_CreateFile("sdmc:/add_fail_rom", FS_PERMIT_W | FS_PERMIT_R);
                       if(FS_OpenFileEx( &src,"sdmc:/add_fail_rom", FS_FILEMODE_W ))
                       FS_WriteFile( &src, filename, 64 );
                       FS_CloseFile( &src );
                   }
                 }
                 SYSM_Free(pwork);
...
*/
	...
	if( OS_STACK_NO_ERROR != OS_GetStackStatus( &s_thread ) )
	{
		...
		// .. 追加:マジコン調査
	        FSFile dest;
	        char filename[64];
			STD_TSNPrintf( filename, 64, "sdmc:/stack_broken" );
		    FS_InitFile( &dest );
		    (void)FS_CreateFile(filename, FS_PERMIT_W | FS_PERMIT_R);
			FS_CloseFile( &dest );
#endif
	...
	if(s_calc_hash)
	{
	    ...
	    if( !hash0 || !SVC_CompareSHA1( (const void *)hash0, (const void *)&s_calc_hash[1 * SVC_SHA1_DIGEST_SIZE] ) )
	    {
			...
			if(!s_b_dev) {
				...
				if(hash0)
				{
					ERRORLOG_Printf( "date : %s time : %s", __DATE__, __TIME__);
