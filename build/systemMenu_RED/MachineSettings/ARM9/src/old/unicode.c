...
void ExSJIS_BEtoUTF16_LE(u8 *sjisp, u16 *unip, u16 length)
{
	...
	while( (*sjisp) && (length-- > 0) ) {
		if( ((*sjisp >= SJIS_HIGHER_CODE1_MIN) && (*sjisp <= SJIS_HIGHER_CODE1_MAX))
		  ||((*sjisp >= SJIS_HIGHER_CODE2_MIN) && (*sjisp <= SJIS_HIGHER_CODE2_MAX)) ) {	// SJISÇ©ÅH
...
void ExUTF16_LEtoSJIS_BE(u8 *sjisp, u16 *unip, u16 length)
{
	...
	while( (*unip) && (length-- > 0) ) {
		...
		}else if(   (code >= 0xff01) && (code <= 0xffe5) )
		{
			...
			}else {													// Unicode = 0xff01 - 0xffe5 ÇÕÅA SJIS = 0x8143 - 0x8197Ç…îzíu
		...
		}else if(   (code >= 0x3000) && (code <= 0x30fe) )
		{
			...
			}else {													// Unicode = 0x3000 - 0x30fe ÇÕÅA SJIS = 0x8140 - 0x81acÇ…îzíu
		...
		if( sjis_le & 0xff00) {										// ASCIIÉRÅ[ÉhÇ≈Ç»ÇØÇÍÇŒÅAÉrÉbÉOÉGÉìÉfÉBÉAÉìå`éÆÇ≈äiî[ÅB
			*sjisp++ = (u8)(sjis_le >> 8);
		}
		*sjisp++ = (u8)(sjis_le);
	}
