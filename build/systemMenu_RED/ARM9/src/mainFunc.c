...
// global variable-------------------------------------------------------------
GXOamAttr	oamBakS[ 128 ]    ATTRIBUTE_ALIGN(32);					// OAM バックアップ
GXOamAttr	oamBakS[ 128 ]    ATTRIBUTE_ALIGN(32);					// OAM バックアップ
u16			bgBakM[ 32 * 24 ] ATTRIBUTE_ALIGN(32);					// BG  バックアップ
u16			bgBakS[ 32 * 24 ] ATTRIBUTE_ALIGN(32);					// BG  バックアップ
...
void InitBG(void)
{
	...
	// メインLCD
	{
		...
		MI_DmaFill32( 3, (void *)HW_OAM, 192, HW_OAM_SIZE );		// OAM      クリア		// NitroSDKではcrt0.cでクリアしている。
...
/*
	{
	    ...
	    GXS_SetVisiblePlane( GX_PLANEMASK_BG1 );					// display only BG #0
	    ...
	    G2S_BG1Mosaic( FALSE );
		MI_DmaFill32( 3, (void *)HW_DB_OAM, 192, HW_OAM_SIZE );		// OAM      クリア
		MI_CpuClearFast( (void *)HW_DB_BG_VRAM,  0x20000 );			// BG -VRAM クリア
		MI_CpuClearFast( (void *)HW_DB_OBJ_VRAM, 0x20000 );			// OBJ -VRAM クリア
		MI_CpuCopyFast( myPlttData, (void *)HW_DB_BG_PLTT, sizeof(myPlttData) );	//  BGパレット   セット
		MI_CpuCopyFast( myPlttData, (void *)HW_DB_BG_PLTT, sizeof(myPlttData) );	// OBJパレット   セット
	}
*/
}
...
{
#ifndef __TP_OFF
#ifdef __TP_CALIBLATE_ONLY_ENABLE
	return;
#endif	/* __TP_CALIBLATE_ONLY_ENABLE */
...
// ロゴメニューの直接ブート時のキャンセルのみタッチパネルを有効にする。
...
void SetBannerIconOBJ( BannerFileV1 *bannerp )
{
	...
	G2_SetOBJAttr( (GXOamAttr*)&oamBakS[ 1 ],						// OAM pointer
					32,												// X position
					32,												// Y position
					0,												// Priority
...
void DrawMenu(u16 nowCsr, const MenuComponent *menu)
{
	...
	for( i = 0; i < menu->num; i++) { 
		if(i == nowCsr)	color = menu->select_color;
		else			color = menu->normal_color;
		
		PutStringUTF16( menu->pos_x, (int)menu->pos_y + i * menu->next_y_num, color, (const u16 *)(menu->str_elem)[i] );
//		(void)DrawStringSJIS( menu->pos_x, (u16)(menu->pos_y + i * menu->next_y_num), color, (menu->str_elem)[i]);
	}
}
...
BOOL SelectMenuByTp(u16 *nowCsr, const MenuComponent *menu)
{
	...
	lt_x = (u16)(menu->pos_x * 8);									// メニュー要素先頭のLCD座標を算出
	lt_y = (u16)(menu->pos_y * 8 - 4);
	rb_x = (u16)((menu->pos_x + menu->name_length) * 8);
	rb_y = (u16)(menu->pos_y * 8 + 12);								// ※Y座標は±4のマージン
	
	if(tpd.disp.touch)	target = &tpd.disp;
	else				target = &tpd.last;
	
//	(void)DrawDecimalSJIS( 20 , 4, WHITE, &csr_old, 2, 2);
//	(void)DrawDecimalSJIS( 20 , 5, WHITE, &same_csr_count, 2, 2);
//	(void)DrawDecimalSJIS( 20 , 6, WHITE, &detach_count, 2, 2);
	
	for(i = 0; i < menu->num; i++) {
		if(tpd.disp.touch) {										// タッチパネルがメニューの要素上でタッチされているなら、
			if(InRangeTp(lt_x, lt_y, rb_x, rb_y, target)) {
		...
		lt_y += (u16)(menu->next_y_num * 8);
		rb_y += (u16)(menu->next_y_num * 8);
	}
	same_csr_count = 0;
	return FALSE;
}
...
static void GetAndDrawRtcDataCore( BOOL forceGetFlag )
{
	...
	if( forceGetFlag || ( s_vcount++ == 60 ) ) {
		...
		// 前RTC情報の消去
		{
			...
			PrintfSJIS( s_rtcPos.date_x,  s_rtcPos.date_y, TXT_COLOR_WHITE, "%d/%02d/%02d[%3s]",
						year,
						date_old.month,
						date_old.day,
						g_strWeek[ date_old.week ]
						);
			PrintfSJIS( s_rtcPos.time_x,  s_rtcPos.time_y, TXT_COLOR_WHITE, "%02d:%02d:%02d",
						time_old.hour,
						time_old.minute,
						time_old.second
						);
		...
		{
			year = s_rtcDate.year + 2000;
			PrintfSJIS( s_rtcPos.date_x,  s_rtcPos.date_y, TXT_COLOR_BLACK, "%d/%02d/%02d[%3s]",
						year,
						s_rtcDate.month,
						s_rtcDate.day,
						g_strWeek[ s_rtcDate.week ]
						);
			PrintfSJIS( s_rtcPos.time_x,  s_rtcPos.time_y, TXT_COLOR_BLACK, "%02d:%02d:%02d",
						s_rtcTime.hour,
						s_rtcTime.minute,
						s_rtcTime.second
						);
