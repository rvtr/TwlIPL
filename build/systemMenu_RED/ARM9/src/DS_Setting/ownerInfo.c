...
void SEQ_OwnerInfo_init(void)
{
	...
	if(ow == NULL) {
		ow = OS_Alloc(sizeof(OwnerWork));							// オーナー情報編集用ワークの確保
#ifdef __SYSM_DEBUG
	...
	if((pad.trg & PAD_BUTTON_A) || (tp_select)) {					// メニュー項目への分岐
		...
		switch(ow->sel) {
			case 0:
				...
				nowProcess = SEQ_InputNickname;
				break;
			case 1:
				SEQ_InputBirthday_init();
				nowProcess = SEQ_InputBirthday;
				break;
			case 2:
				SEQ_InputFavoriteColor_init();
				nowProcess = SEQ_InputFavoriteColor;
				break;
		}
...
static void ReturnMenu(int save_flag)
{
	...
	SEQ_OwnerInfo_init();
	nowProcess = SEQ_OwnerInfo;										// オーナー情報編集に戻る
}
