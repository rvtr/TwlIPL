...
/*
	...
	Revision 1.19  2004/08/25 02:21:40  Yosiokat
	・1.14のコメントは間違いのため、再コミット。
	・small fix.
	
	Revision 1.18  2004/08/25 02:15:51  Yosiokat
	タグred_ipl2_2004_08_24_pp2_isdbg_fixの削除。
	
	Revision 1.17  2004/08/20 11:35:29  Yosiokat
	small fix.
	
	Revision 1.16  2004/08/18 07:17:29  Yosiokat
	上下LCDをターゲットにして、別個に初期化できるよう変更。
	
	Revision 1.15  2004/08/17 07:52:03  Yosiokat
	・SetTargetScreenSJISを追加して、上下LCDのどちらにも文字表示が可能になるよう変更。
	
	Revision 1.14  2004/08/16 10:03:08  Yosiokat
	GetStrEntryで範囲外のhandleが与えられた際にNULLを返すよう変更。
	
	Revision 1.13  2004/08/13 07:32:58  Yosiokat
	small fix.
	
	Revision 1.12  2004/08/13 04:33:18  Yosiokat
	small fix.
	
	Revision 1.11  2004/08/10 07:02:10  yosiokat
	small fix.
	
	Revision 1.10  2004/08/07 05:44:44  Yosiokat
	・SJIS文字列表示関数を引数でハンドルを指定しない仕様に変更する。
	・上記変更に対応して、クリア関数も仕様変更。
	
	Revision 1.9  2004/07/29 04:58:42  Yosiokat
	構造体のパディングを明示的に追加。
	
	Revision 1.8  2004/07/13 00:31:48  Yosiokat
	・サブLCD側のVRAMを対象にするよう変更。
	
	Revision 1.7  2004/06/15 13:09:10  Yosiokat
	・MY_PrintfをOS_Printfに。
	
	Revision 1.6  2004/06/06 06:28:24  Yosiokat
	OS_PrintfをOS_Printfに。
	
	Revision 1.5  2004/06/06 02:39:31  Yosiokat
	SJISコード判定用の定数定義をfont.hに移動。
	
	Revision 1.4  2004/05/31 11:53:23  yosiokat
	__DEBUGを__PRINTに変更。
	
	Revision 1.3  2004/05/26 05:07:34  Yosiokat
	DrawStringSJISで空文字列を指定した際に異常動作するバグを修正。
...
static void		*AllocMemoryFromVram( int size );
static void		FreeMemoryToVram( void *p );

static u16		DrawStringSJIS_Core(u16 x, u16 y, u16 color, const void *strp, int handleIndex);
static int		CalcHandleIndex(u32 dataAddr, int index);
...
static u16 DrawStringSJIS_Core(u16 x, u16 y, u16 color, const void *strp, int handleIndex)
{
	...
	// ハンドル取得
	if( !GetEmptyStrEntry( (u32)handleIndex, &handle ) ) {
	...
	sEntryp->dataAddr	= (u32)handleIndex;
	
	// VRAMキャラメモリの動的確保
	if(newEntry > 0) {
		sEntryp->charMemp = AllocMemoryFromVram( charNum * NITRO_CHAR_SIZE * 2 );
...
{
	if(index) {
		index += HANDLE_INDEX + (int)dataAddr;							// インデックスを加算した場合は、他のデータのアドレスと被らないよう、NITROメモリマップ外になるようオフセットを加算。
	}else {
		index =  (int)dataAddr;
	}
	return index;
...
u16 DrawStringSJIS(u16 x, u16 y, u16 color, const void *strp)
{
	return DrawStringSJIS_Core( x, y, color, strp, (int)strp);			// index = strp でCoreをコール
}
...
u16 DrawHexSJIS(u16 x, u16 y, u16 color, const void *hexp, u16 figure)
{
	...
	return DrawStringSJIS_Core(x, y, color, strp, (int)hexp);
}
